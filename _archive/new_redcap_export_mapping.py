...
class SponsoredProjectRunner:
    ...
    _GITHUB_REPO = "https://github.com/Sage-Bionetworks/GENIE-Sponsored-Projects"
    ...
    def createMetaDiagnosisDf(self, timelineDf):
        ...
        metaDiagnosisDf["PATIENT_ID"] = timelineDf["PATIENT_ID"].drop_duplicates()
        metaDiagnosisDf["START_DATE"] = 0
        metaDiagnosisDf["EVENT_TYPE"] = "STATUS"
        metaDiagnosisDf["STATUS"] = "Metastatic Diagnosis"
        metaDiagnosisDf = metaDiagnosisDf[~metaDiagnosisDf["START_DATE"].isnull()]
    ...
    def createGeneMatrixDf(self, clinicalDf, cnaSamples, usedEnt):
        ...
        if not self.staging:
            self.syn.store(fileEnt, used=usedEnt.id, executed=self._GITHUB_REPO)
    ...
    def writeClinicalFile(self, clinicalDf, redCapToCbioMappingDf, sampleType):
        ...
        """
        assert sampleType in [
            "patient",
            "sample",
        ], "sample type must be patient or sample"
        labels = [
            str(
                redCapToCbioMappingDf["labels"][
                    redCapToCbioMappingDf["cbio"] == col
                ].values[0]
            )
            for col in clinicalDf
        ]
        descriptions = [
            str(
                redCapToCbioMappingDf["description"][
                    redCapToCbioMappingDf["cbio"] == col
                ].values[0]
            )
            for col in clinicalDf
    ...
    def run(self):
        ...
        for field, instrument in zip(mappingDf.genie_field_name, mappingDf.instrument):
            ...
            if "#" in field:
                ...
                newDataFrame = pd.DataFrame(
                    {"genie_field_name": field, "instrument": instrument}, index=[0]
                )
        ...
        labeledDf["redcap_data_access_group"][
            labeledDf["redcap_data_access_group"] == "hop"
        ] = "JHU"
        labeledDf["redcap_data_access_group"] = labeledDf[
        ...
        finalPatientDf = self.configureClinicalDf(patientDf, redCapToCbioMappingDf)
        patient_date_col = [
        ...
        finalPatientDf[patient_date_col] = finalPatientDf[patient_date_col].applymap(
            change_days_to_months
        )
        ...
        finalSampleDf = self.configureClinicalDf(sampleDf, redCapToCbioMappingDf)

        sample_date_cols = ["SAMPLE_DATE_INT", "AGE_AT_SEQ_REPORT"]
        final_sampledf_datesdays = finalSampleDf.copy()
        finalSampleDf[sample_date_cols] = finalSampleDf[sample_date_cols].applymap(
            change_days_to_months
        )
        # Fill in ONCOTREE_CODE
        finalSampleDf["ONCOTREE_CODE"] = [
            genie_clinicalDf["ONCOTREE_CODE"][
                genie_clinicalDf["SAMPLE_ID"] == sample
            ].values[0]
        ...
        if mergedClinicalDf.get("SAMPLE_ID") is not None:
            ...
            notFoundSamples = mergedClinicalDf["SAMPLE_ID"][
                ~mergedClinicalDf["SAMPLE_ID"].isin(genie_clinicalDf["SAMPLE_ID"])
            ]
        ...
        if not self.staging:
            ...
            patientEnt = self.syn.store(
                patientFileEnt, used=labelledEnt.id, executed=self._GITHUB_REPO
            )
            ...
            sampleEnt = self.syn.store(
                sampleFileEnt, used=labelledEnt.id, executed=self._GITHUB_REPO
            )

        treatmentDf = labeledDf[treatmentCols]
        treatmentRows = (
            labeledDf.redcap_repeat_instrument == "Treatment Information Detailed"
        )
        treatmentDf = treatmentDf[treatmentRows]
        finalTimelineDf = self.makeTimeLineDf(treatmentDf, final_patientdf_datesdays)
        ...
        specimenDf = self.createSpecimenDf(
            final_sampledf_datesdays, final_patientdf_datesdays
        )
        specimenDf = specimenDf[
            specimenDf["SAMPLE_ID"].isin(genie_clinicalDf["SAMPLE_ID"])
        ]
        ...
        if not self.staging:
            ...
            self.syn.store(fileEnt, used=labelledEnt.id, executed=self._GITHUB_REPO)
        ...
        index = 0
        mafpath = "{}/data_mutations_extended.txt".format(self._SPONSORED_PROJECT)
        ...
        if not self.staging:
            self.syn.store(
                fileEnt, used=centerMafSynIdsDf.id.tolist(), executed=self._GITHUB_REPO
            )
        ...
        withMergedHugoSymbol = withMergedHugoSymbol.append(
            pd.Series(finalSampleDf["SAMPLE_ID"])
        )
        cnaSamples = []
        ...
        if not self.staging:
            self.syn.store(
                fileEnt, used=centerCNASynIdsDf.id.tolist(), executed=self._GITHUB_REPO
            )
        ...
        if not fusions_df.empty:
            ...
            if not self.staging:
                self.syn.store(fileEnt, used=fusion.tableId, executed=self._GITHUB_REPO)
        ...
        if not seg_df.empty:
            ...
            if not self.staging:
                self.syn.store(fileEnt, used=seg.tableId, executed=self._GITHUB_REPO)
