...
class Akt1(sp_redcap_export_mapping.SponsoredProjectRunner):
    ...
    _DATES = [
        ...
        "primary_dx_date_int",
        "lrr_date_int",
        "mets_disease_date_int",
        "sample_date_int_1",
        "sequence_report_date_int_1",
        "sequence_report_date_int_1_static",
        "sample_date_int_2",
        "sample_date_int_2_static",
        "sequence_report_date_int_2",
        "sequence_report_date_int_2_static",
        "sequence_report_date_int_3_static",
        "OS_MONTHS",
        "OS_MONTHS_PRIMARY",
    ]
    ...
    def addOSMonths(self, sponsoredProject_mapped_df):
        ...
        sponsoredProject_mapped_df["OS_MONTHS"] = (
            ...
            - sponsoredProject_mapped_df["mets_disease_date_int"]
        )
    ...
    def createNullPatients(self, sponsoredProject_mapped_df, tempIdMappingDf):
        print(
            "RENAMING %s NULL PATIENTS"
            % sum(sponsoredProject_mapped_df["genie_patient_id"].isnull())
        )
        # Create temp patient Id
        allNullPatients = sponsoredProject_mapped_df[
            ["record_id", "redcap_data_access_group", "genie_patient_id"]
        ][sponsoredProject_mapped_df["genie_patient_id"].isnull()]
        temporaryIds = allNullPatients.apply(
            lambda x: self.createTemporaryGenieId(x, tempIdMappingDf), axis=1
        )
        if sponsoredProject_mapped_df["genie_patient_id"].isnull().any():
            sponsoredProject_mapped_df["genie_patient_id"][
                sponsoredProject_mapped_df["genie_patient_id"].isnull()
            ] = temporaryIds
        assert (
            sum(sponsoredProject_mapped_df["genie_patient_id"].isnull()) == 0
        ), "Make sure there are no null genie patient Ids"

        sponsoredProject_mapped_df[
            "genie_patient_id"
        ] = sponsoredProject_mapped_df.apply(
            lambda x: self.checkGenieId(
                x, "redcap_data_access_group", "genie_patient_id"
            ),
            axis=1,
        )
    ...
    def makeTimeLineDf(self, redCapExportDf, therapyRange=18):
        ...
        for therapyNumber in range(1, therapyRange):
            therapyCols = [
                i for i in redCapExportDf if "therapy%d_" % therapyNumber in i
            ]
            ...
            THERAPY_DRUG_DISCONTINUE.extend(
                [i for i in therapyCols if "discontinue" in i]
            )
            THERAPY_DRUG_REASON.extend([i for i in therapyCols if "reason" in i])
            THERAPY_COMBO_YN.extend(
                [i for i in therapyCols if "combo_yn" in i]
                * len([i for i in therapyCols if "start_int" in i])
            )
            THERAPY_COMBO_NUM.extend(
                [i for i in therapyCols if "combo_num" in i]
                * len([i for i in therapyCols if "start_int" in i])
            )
            TREATMENT_TYPE.extend(
                ["Medical Therapy %d" % therapyNumber]
                * len([i for i in therapyCols if "start_int" in i])
            )
            SUBTYPE.extend(
                ["Chemo/Target/Immuno etc."]
                * len([i for i in therapyCols if "start_int" in i])
            )
        # OVARIAN
        ...
        AGENT.extend(
            [
                ...
                and "discon" not in i
                and "other" not in i
                and "clintrial" not in i
                and "start_int" not in i
                and "end_int" not in i
                and "therapy" not in i
            ]
        )
        ...
        for i in range(len(redCapExportDf)):
            ...
            timelineDF["PATIENT_ID"] = [redCapExportDf["genie_patient_id"][i]] * len(
                START_DATE
            )
            ...
            timelineDF["START_DATE"] = (
                redCapExportDf.iloc[i][START_DATE].reset_index(drop=True)
                - redCapExportDf.iloc[i]["mets_disease_date_int"]
            )
            timelineDF["STOP_DATE"] = (
                redCapExportDf.iloc[i][STOP_DATE].reset_index(drop=True)
                - redCapExportDf.iloc[i]["mets_disease_date_int"]
            )
            ...
            timelineDF["THERAPY_DRUG_CLINTRIAL"] = redCapExportDf.iloc[i][
                THERAPY_DRUG_CLINTRIAL
            ].reset_index(drop=True)
            timelineDF["THERAPY_DRUG_AZD5363"] = redCapExportDf.iloc[i][
                THERAPY_DRUG_AZD5363
            ].reset_index(drop=True)
            timelineDF["THERAPY_DRUG_OTHER"] = redCapExportDf.iloc[i][
                THERAPY_DRUG_OTHER
            ].reset_index(drop=True)
            timelineDF["THERAPY_DRUG_DISCONTINUE"] = redCapExportDf.iloc[i][
                THERAPY_DRUG_DISCONTINUE
            ].reset_index(drop=True)
            timelineDF["THERAPY_DRUG_REASON"] = redCapExportDf.iloc[i][
                THERAPY_DRUG_REASON
            ].reset_index(drop=True)
            timelineDF["THERAPY_COMBO_YN"] = redCapExportDf.iloc[i][
                THERAPY_COMBO_YN
            ].reset_index(drop=True)
            timelineDF["THERAPY_COMBO_NUM"] = redCapExportDf.iloc[i][
                THERAPY_COMBO_NUM
            ].reset_index(drop=True)
    ...
    def getSpecimen(self, getTimelineSpecimen):
        ...
        specimen["START_DATE"] = (
            getTimelineSpecimen.SEQUENCE_REPORT_DATE_INT_STATIC
...
class Erbb2(sp_redcap_export_mapping.SponsoredProjectRunner):
    ...
    _DATES = [
        ...
        "date_first_met_int",
        "sample_date_int_1",
        "seq_report_date_int_1",
        "sample_date_int_2",
        "seq_report_date_int_2",
        "sample_date_int_3",
        "sequence_report_date_int_3",
        "sample_date_int_4",
        "sequence_report_date_int_4",
        "sample_date_int_5",
        "sequence_report_date_int_5",
        "sample_date_int_6",
        "seq_report_date_int_6",
        "sample_date_int_7",
        "seq_report_date_int_7",
        "sample_date_int_8",
        "sequence_report_date_int_8",
        "sample_date_int_9",
        "sequence_report_date_int_9",
        "sample_date_int_10",
        "sequence_report_date_int_10",
        "date_bso_int",
        "OS_MONTHS",
        "OS_MONTHS_PRIMARY",
    ]
    ...
    def createTemporaryGenieId(self, x, tempIdMapping, patientIdCol):
        ...
        if sum(tempIdMapping["uniqueId"] == uniqId) == 0:
            ...
            self.syn.store(
                synapseclient.Table(self.syn.get("syn10164044"), [[uniqId, tempId]])
            )
            return tempId
        else:
            return tempIdMapping[tempIdMapping["uniqueId"] == uniqId][
                "temporaryId"
            ].values[0]
    ...
    def createNullPatients(self, sponsoredProject_mapped_df, tempIdMappingDf):
        ...
        sponsoredProject_mapped_df["redcap_data_access_group"] = [
            i.upper() for i in sponsoredProject_mapped_df["redcap_data_access_group"]
        ]
        allNullPatients = sponsoredProject_mapped_df[
            ["record_id_patient_id", "redcap_data_access_group"]
        ][sponsoredProject_mapped_df["record_id_patient_id"].isnull()]
        temporaryIds = allNullPatients.apply(
            lambda x: self.createTemporaryGenieId(
                x, tempIdMappingDf, "record_id_patient_id"
            ),
            axis=1,
        )
        if not temporaryIds.empty:
            sponsoredProject_mapped_df["record_id_patient_id"][
                sponsoredProject_mapped_df["record_id_patient_id"].isnull()
            ] = temporaryIds
        assert (
            sum(sponsoredProject_mapped_df["record_id_patient_id"].isnull()) == 0
        ), "Make sure there are no null genie patient Ids"
        sponsoredProject_mapped_df[
            "record_id_patient_id"
        ] = sponsoredProject_mapped_df.apply(
            lambda x: self.checkGenieId(
                x, "redcap_data_access_group", "record_id_patient_id"
            ),
            axis=1,
        )
        return (sponsoredProject_mapped_df, temporaryIds)
    ...
    def makeTimeLineDf(self, redCapExportDf, therapyRange=16):
        ...
        for therapyNumber in range(1, therapyRange):
            therapyCols = [
                i
                for i in redCapExportDf
                if (
                    "therapy%d_" % therapyNumber in i
                    or "combo_therapy_yn_%d" % therapyNumber == i
                    or "add_treatment_%d" % therapyNumber == i
                    or "treatment_setting_%d" % therapyNumber == i
                )
            ]
            START_DATE.extend([i for i in therapyCols if "start_int" in i])
            STOP_DATE.extend([i for i in therapyCols if "end_int" in i])
            AGENT.extend(
                [
                    i
                    for i in therapyCols
                    if len(i.split("_")) == 2
                    and "response" not in i
                    and "ctdrug" not in i
                ]
            )
            THERAPY_DRUG_OTHER.extend([i for i in therapyCols if "other" in i])
            ...
            THERAPY_DRUG_REASON.extend([i for i in therapyCols if "reason" in i])
            THERAPY_COMBO_YN.extend(
                [i for i in therapyCols if "combo_therapy_yn" in i]
                * len([i for i in therapyCols if "start_int" in i])
            )
            THERAPY_COMBO_NUM.extend(
                [i for i in therapyCols if "combo_num" in i]
            ...
            THERAPY_RESPONSE.extend(
                [i for i in therapyCols if "response" in i]
                * len([i for i in therapyCols if "start_int" in i])
            )
            ADD_TREATMENT.extend(
                [i for i in therapyCols if "add_treatment" in i]
                * len([i for i in therapyCols if "start_int" in i])
            )
            TREATMENT_SETTING.extend(
                [i for i in therapyCols if "treatment_setting" in i]
                * len([i for i in therapyCols if "start_int" in i])
            )
        EVENT_TYPE = ["TREATMENT"] * len(AGENT)
        ...
        for i in range(len(redCapExportDf)):
            ...
            timelineDF["PATIENT_ID"] = [
                redCapExportDf["record_id_patient_id"][i]
            ] * len(START_DATE)
            if not pd.isnull(redCapExportDf.iloc[i]["date_first_met_int"]):
                timelineDF["START_DATE"] = [
                    start
                    if pd.isnull(start)
                    else int(start) - int(redCapExportDf.iloc[i]["date_first_met_int"])
                    for start in redCapExportDf.iloc[i][START_DATE].reset_index(
                        drop=True
                    )
                ]
                timelineDF["STOP_DATE"] = [
                    ...
                    if pd.isnull(end)
                    else int(end) - int(redCapExportDf.iloc[i]["date_first_met_int"])
                    for end in redCapExportDf.iloc[i][STOP_DATE].reset_index(drop=True)
                ]
            else:
    ...
    def getSpecimen(self, getTimelineSpecimen):
        specimen = pd.DataFrame()
        specimen["PATIENT_ID"] = getTimelineSpecimen["PATIENT_ID"]
        getTimelineSpecimen = getTimelineSpecimen[
            ~getTimelineSpecimen.SEQUENCE_REPORT_DATE_INT_STATIC.isnull()
        ]
        getTimelineSpecimen = getTimelineSpecimen[
            ~getTimelineSpecimen.METS_DISEASE_DATE_INT.isnull()
        ]
        specimen[
            "START_DATE"
        ] = getTimelineSpecimen.SEQUENCE_REPORT_DATE_INT_STATIC.astype(
            int
        ) - getTimelineSpecimen.METS_DISEASE_DATE_INT.astype(
            int
        )
        specimen["EVENT_TYPE"] = "SPECIMEN"
        specimen["SAMPLE_ID"] = getTimelineSpecimen["SAMPLE_ID"]
...
class Fgfr4(new_redcap_export_mapping.SponsoredProjectRunner):
    ...
    def makeTimeLineDf(self, treatmentDf, finalPatientDf, therapyRange=5):
        ...
        for therapyNumber in range(1, therapyRange):
            therapyCols = [
                i for i in treatmentDf if "therapy_drug%d" % therapyNumber in i
            ]
            startCols = [i for i in therapyCols if "start_int" in i]
            START_DATE.extend(startCols)
            STOP_DATE.extend([i for i in therapyCols if "end_int" in i])
            AGENT.extend([i for i in therapyCols if "name" in i and "other" not in i])
            RXNORM_ID.extend(
                [i for i in therapyCols if i == "therapy_drug%d" % therapyNumber]
            )
        ...
        for i in range(len(treatmentDf)):
            timelineDF = pd.DataFrame()
            timelineDF["PATIENT_ID"] = [treatmentDf["patient_id"].iloc[i]] * len(
                START_DATE
            )
            timelineDF["START_DATE"] = treatmentDf.iloc[i][START_DATE].reset_index(
                drop=True
            )
            timelineDF["STOP_DATE"] = treatmentDf.iloc[i][STOP_DATE].reset_index(
                drop=True
            )
            ...
            timelineDF["TREATMENT_TYPE"] = TREATMENT_TYPE
            timelineDF["SUBTYPE"] = SUBTYPE
            timelineDF["AGENT"] = treatmentDf.iloc[i][AGENT].reset_index(drop=True)
            timelineDF["RXNORM_ID"] = treatmentDf.iloc[i][RXNORM_ID].reset_index(
                drop=True
            )
            timelineDF["THERAPY_DRUG_OTHER"] = treatmentDf.iloc[i][
                THERAPY_DRUG_OTHER
            ].reset_index(drop=True)
            timelineDF["THERAPY_DRUG_DISCONTINUE"] = treatmentDf.iloc[i][
                THERAPY_DRUG_DISCONTINUE
            ].reset_index(drop=True)
            timelineDF["THERAPY_DRUG_REASON"] = treatmentDf.iloc[i][
                THERAPY_DRUG_REASON
            ].reset_index(drop=True)
            ...
            timelineDF["TREATMENT_SETTING"] = treatmentDf.iloc[i][
                TREATMENT_SETTING
            ].reset_index(drop=True)
            timelineDF["THERAPY_RESPONSE"] = treatmentDf.iloc[i][
                THERAPY_RESPONSE
            ].reset_index(drop=True)
            timelineDF["LINE_START"] = treatmentDf.iloc[i][LINE_START].reset_index(
                drop=True
            )
            timelineDF["REGIMEN_NAME"] = treatmentDf.iloc[i][REGIMEN_NAME].reset_index(
                drop=True
            )
            timelineDF["CLINICAL_TRIAL"] = treatmentDf.iloc[i][
                CLINICAL_TRIAL
            ].reset_index(drop=True)
            timelineDF["CENTER"] = treatmentDf.iloc[i][CENTER].reset_index(drop=True)
        ...
        date_met_int = [
            float(
                finalPatientDf["MET_DX_DATE_INT"][
                    finalPatientDf["PATIENT_ID"] == patient
                ].values[0]
            )
    ...
    def createSpecimenDf(self, sampleDf, patientDf):
        ...
        clinicalDf = clinicalDf[~clinicalDf.DATE_FIRST_DISTANT_MET_INT.isnull()]
        specimen = pd.DataFrame()
        specimen["PATIENT_ID"] = clinicalDf["PATIENT_ID"]
        specimen["SAMPLE_ID"] = clinicalDf["SAMPLE_ID"]
        specimen["START_DATE"] = clinicalDf.AGE_AT_SEQ_REPORT.astype(
            int
