...
def get_cbioportal_upload_folders(syn: Synapse, staging_release_folder: str, cohort: str, release: str) -> dict:
    """Create case lists and release folder"""
    sp_data_folder = syn.store(
        Folder(cohort, parentId=staging_release_folder)
    )
    ...
    # clinical files
    release_folder = syn.store(
        Folder("cBioPortal_files", parent=release_folder)
    )
    case_lists = syn.store(Folder("case_lists", parent=release_folder))
...
#         )
#         release_folder = self.syn.store(Folder(self.bpc_config.release, parent=sp_data_folder))
#         # Store in cBioPortal files because there is another folder for
#         # clinical files
